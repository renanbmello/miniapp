...
async function getBlockWithCache(
...
async function checkProposalContract(
    provider: JsonRpcProvider,
    address: string
): Promise<boolean> {
    try {
    ...
    try {
        ...
        return {
            ...
            latestProposal: {
                ...
                votingDeadline:
                    proposal.votingDeadline?.toNumber() || undefined,
...
export async function POST(req: NextRequest) {
    try {
        ...
        for (
            let i = blockNumber;
            i > blockNumber - 100 && transactions.length < 10;
            i--
        ) {
            ...
            const blockTxs =
                block.transactions as unknown as TransactionResponse[];
        ...
        for (const tx of transactions) {
            ...
            const matchesProposalSignature = Object.values(
                PROPOSAL_SIGNATURES
            ).some((sig) => txData.startsWith(sig));

            if (
                matchesProposalSignature ||
                (await checkProposalContract(provider, tx.to))
            ) {
        ...
        // Get proposal data
        const proposalDataPromises = Array.from(proposalContracts).map(
            (address) => getProposalData(provider, address)
        );

        const proposalData = (await Promise.all(proposalDataPromises)).filter(
            (data): data is ProposalData => data !== null
        );
...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!userAddress) {
            const response = await JsonService.readJson<TransactionsResponse>(
                "transactions.json"
            );
            ...
            const proposals: Proposal[] = response.data.votes.map((vote) => ({
                ...
                scores_total:
                    vote.proposal.scores?.reduce((a, b) => a + b, 0) || 0,
                scores_state: "final",
...
export async function GETbyAddress(
    ...
    try {
        const response = await JsonService.readJson<ProposalsResponse>(
            "proposals.json"
        );

        const addressProposals = response.data.proposals.filter(
            (proposal) =>
